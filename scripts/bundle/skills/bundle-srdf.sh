...
# concurrently.
SNAPSHOT_BUILD_DEPS_DIR="${SRDF_SNAPSHOT_BUILD_DEPS_DIR:-$REPO_ROOT/tmp/srdf-snapshot-build}"
...
if [ "$MODE" = "check" ] && [ -L "$CADGEN_RUNTIME_DIR" ]; then
  ...
  echo "SRDF skill is in development symlink layout; production package freshness is checked on build-test/main."
  exit 0
fi
...
if [ "$MODE" = "check" ]; then
  ...
  if [ "$stale" -ne 0 ]; then
    echo "" >&2
    echo "Run scripts/bundle/bundle-skill.sh srdf and commit the updated production outputs." >&2
    exit 1
  fi
