...
build_runtime() {
  local target_dir="$1"
  rm -rf "$target_dir"
  mkdir -p "$target_dir" "$target_dir/backend"
  ...
  # Keep the fork's agent-aware launcher alongside the upstream production
  # launcher. It reuses compatible viewers and falls back to the bundled Node
  # backend without changing the upstream `npm start` contract.
  (
    cd "$REPO_ROOT"
...
require_path "$VIEWER_DIR/src/server/server.mjs" "viewer agent Node backend"
require_path "$VIEWER_DIR/scripts/start-agent-viewer.mjs" "viewer agent launcher"
