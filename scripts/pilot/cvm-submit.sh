...
case "$mode" in
    pilot)
        ...
        ;;
    provider-free)
        [[ $# -eq 3 && ( "$2" == "installed-plugin" || "$2" == "agent-surface-mcp-injection" || "$2" == "agent-surface-mcp-direct-injection" || "$2" == "agent-surface-mcp-ephemeral-differential" || "$2" == "workspace-repair-chain" || "$2" == "workspace-repair-chain-exhaustion" ) ]] || usage
        scenario="$2"
        group="$3"
