---
name: cvm-pull
description: Upload CVM new pilot outputs to S3 (verified, then clean CVM local).
  Mac reads via existing rclone mount at ~/threed-code/ericzyma/text-to-cad/outputs/.
  Trigger: "cvm-pull", "拉 outputs", "sync from CVM", "拿结果", "pull results".
---
...
- **方案 α — 上传 S3，不写 Mac 本地磁盘**：CVM 用 `aws s3 cp --recursive` 传到
  `s3://arcwm-code-us-west-2/ericzyma/text-to-cad/outputs/<exp>/`；Mac 通过
  现有 rclone mount `~/threed-code/ericzyma/text-to-cad/outputs/` 看。
- **Method W — 只上传 S3 里还没有的 exp**：脚本先 ssh 列 CVM exp dir 名，
  `comm -23` 过滤掉 mount 里已有，只对差集 upload。**依赖 pilot dir immutable 假设**。
...
- 假定 CVM 上 `~/text-to-cad/outputs/` 存在；空则脚本 exit 0（"nothing to do"）。
...
- **重拉一个 exp**：`rm -rf $MOUNT_PATH/<exp>` OR `aws s3 rm --recursive $S3_PREFIX/<exp>/`
  后再跑 `/cvm-pull`。若 CVM local 已清、S3 也删了，就只能靠 pilot 重跑
  （另一个流程）。
...
- 下一步提示：`/pilot-review outputs/<glob>/`（推荐指向刚上传的那批；`outputs/`
  是 symlink 指向 mount）
