...
**Snapshot runtime.** `scripts/snapshot` consumes the job's read-only Browser Runtime capability. If that capability is missing or rejected, stop and report the runtime failure; the outer runner owns browser provisioning. Do not install Playwright/Chromium or start a local browser.
...
python scripts/canonical-build build \
  ...
  --input source/declared-sidecar.json \
