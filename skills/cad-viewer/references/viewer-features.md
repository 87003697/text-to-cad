...
- Copied references carry their file: the Viewer prefixes every copied ref with the shortest
  path suffix that names that file uniquely (`bracket.step.py#o1.2.f1`, or
  `starship/super_heavy.step.py#o1.3` where a filename is not unique), so a ref pasted into a
  prompt still says which model it belongs to. The CAD skill's
  `references/inspection-and-validation.md` documents how to split the prefix off and resolve
  it before calling the CLIs. Bare `#...` refs remain valid everywhere.
