...
- `.stl`, `.3mf`, `.glb`: mesh viewing with orbit/pan/zoom, screenshots, theme controls, and solid/wireframe display where available.
...
- File sheet: open the right sheet for file-specific controls such as STEP tree/parameters, G-code layers/detail/travel, DXF thickness/bends, URDF/SRDF/SDF joints and metadata, plus the Display tab.
...
- Theme sidebar: a "Preset" dropdown (System, the built-in presets, then None once you have edited something) followed by the settings groups. Presets are read-only and there is only one custom theme: editing any setting writes it into that single custom slot, and picking a preset again is how you reset. There is no save, restore, rename, or delete.
