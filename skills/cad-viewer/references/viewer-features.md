...
- Display/appearance: use the file sheet and theme menu for solid/wireframe/x-ray modes, surface colors, edge visibility/detail, highlight styling, backdrop, floor/grid, lighting, theme presets, and color mode.
- Clip: the STEP file sheet's dedicated "Clip" tab sections the model with an axis-aligned plane — X/Y/Z position sliders plus Flip and Reset.
- Exploded view: the STEP file sheet's dedicated "Exploded" tab pulls an assembly apart — an Enable/Disable button (toggles the Amount scrub to/from zero), an Amount scrub, an Automatic/Custom layout switch, a Direction dropdown (Auto/X/Y/Z/Radial), Reverse, Spread, Detail, Order, and explode-line controls, where Custom lets you edit per-part moves.
