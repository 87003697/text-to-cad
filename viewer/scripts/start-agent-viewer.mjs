...
export function buildAgentStartCommand({
  ...
  const bundledServer = path.join(resolvedPackageRoot, "backend", "server.mjs");
  const serverEntrypoint = fs.existsSync(bundledServer)
    ? bundledServer
...
export async function runAgentStart(options = {}) {
  ...
  return executeAgentStartLaunch(launch, options);
}
