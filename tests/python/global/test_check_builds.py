...
class CheckBuildsTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        tracked_file = self.repo / "skills/example/SKILL.md"
        tracked_file.write_text("# Example\n", encoding="utf-8")
        subprocess.run(
    ...
    def test_rejects_tracked_symlink_outside_generated_outputs(self) -> None:
        link = self.repo / "skills/example/references/shared"
        os.symlink("../../example/SKILL.md", link)
        subprocess.run(
            ["git", "add", "skills/example/references/shared"],
            cwd=self.repo,
            check=True,
        )
        ...
        self.assertNotEqual(result.returncode, 0)
        self.assertIn("Published skills must not contain symlinks.", result.stderr)
        self.assertIn(
            "skills/example/references/shared",
            result.stderr,
        )

    def test_ignores_untracked_development_dependency_symlink(self) -> None:
